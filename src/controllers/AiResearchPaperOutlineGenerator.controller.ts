...
export const AiResearchPaperOutlineGenerator = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (!researchTopic?.trim().length) {
    res.status(StatusCodes.BAD_REQUEST).json({ error: "Please provide the research topic." });
    return;
  }
  
  if (!wordCount && wordCount <= 0) {
    res.status(StatusCodes.BAD_REQUEST).json({ error: "Please specify the word count." });
    return;
  }

  if (!citationStyle?.trim().length) {
    res.status(StatusCodes.BAD_REQUEST).json({ error: "Please specify citation style." });
    return;
  }
