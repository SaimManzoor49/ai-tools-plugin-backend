...
export const AiHookGenerator = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (!topic?.trim().length) {
    res.status(StatusCodes.BAD_REQUEST).json({ error: "Please specify the topic for your hook." });
    return;
  }
  if (!hookStyle?.trim().length) {
    res.status(StatusCodes.BAD_REQUEST).json({ error: "Please specify hook style." });
    return;
  }
