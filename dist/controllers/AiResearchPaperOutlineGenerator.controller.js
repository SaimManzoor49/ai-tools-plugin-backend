...
exports.AiResearchPaperOutlineGenerator = void 0;
...
exports.AiResearchPaperOutlineGenerator = (0, express_async_handler_1.default)(async (req, res) => {
    const { researchTopic, wordCount, citationStyle, additionalInstructions = 'no additional instructions', name, siteUrl } = req.body;
    // Validate input
    ...
    if (!researchTopic?.trim().length) {
        res.status(http_status_codes_1.StatusCodes.BAD_REQUEST).json({ error: "Please provide the research topic." });
        return;
    }
    if (!wordCount && wordCount <= 0) {
        res.status(http_status_codes_1.StatusCodes.BAD_REQUEST).json({ error: "Please specify the word count." });
        return;
    }
    if (!citationStyle?.trim().length) {
        res.status(http_status_codes_1.StatusCodes.BAD_REQUEST).json({ error: "Please specify citation style." });
        return;
    }
    try {
        ...
        const dynamicVariables = {
            researchTopic, wordCount, citationStyle, additionalInstructions
        };
