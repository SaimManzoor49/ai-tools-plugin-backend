...
exports.claimCounterClaimGenerator = (0, express_async_handler_1.default)(async (req, res) => {
    ...
    if (!claimOrCounterClaim?.trim().length) {
        res.status(http_status_codes_1.StatusCodes.BAD_REQUEST).json({ error: "Please specify if its claim or counter claim." });
        return;
    }
    ...
    if (!claimType?.trim().length) {
        res.status(http_status_codes_1.StatusCodes.BAD_REQUEST).json({ error: "Please specify the claim type." });
        return;
    }
    if (!tone?.trim().length) {
