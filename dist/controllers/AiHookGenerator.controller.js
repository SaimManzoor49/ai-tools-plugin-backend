...
exports.AiHookGenerator = void 0;
...
exports.AiHookGenerator = (0, express_async_handler_1.default)(async (req, res) => {
    ...
    if (!topic?.trim().length) {
        res.status(http_status_codes_1.StatusCodes.BAD_REQUEST).json({ error: "Please specify the topic for your hook." });
        return;
    }
    if (!hookStyle?.trim().length) {
        res.status(http_status_codes_1.StatusCodes.BAD_REQUEST).json({ error: "Please specify hook style." });
        return;
    }
    try {
        ...
        console.error("Error processing AI Hook Generator request:", error);
