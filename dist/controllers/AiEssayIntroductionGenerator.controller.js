...
exports.AiEssayIntroductionGenerator = void 0;
...
exports.AiEssayIntroductionGenerator = (0, express_async_handler_1.default)(async (req, res) => {
    ...
    if (!essayType?.trim().length) {
        res.status(http_status_codes_1.StatusCodes.BAD_REQUEST).json({ error: "Please specify essay type." });
        return;
    }
    if (!thesisStatement?.trim().length) {
        res.status(http_status_codes_1.StatusCodes.BAD_REQUEST).json({ error: "Please provide thesis statement." });
        return;
    }
